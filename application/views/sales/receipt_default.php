<div id="receipt_wrapper" style="font-size:<?php echo $this->config->item('receipt_font_size'); ?>px">
    ...
    <table id="receipt_items">
        <tr>
            ...
            ?>
                <th style="width:20%;"><?php echo $this->lang->line('sales_item_net'); ?></th>
                <th style="width:20%;"><?php echo $this->lang->line('sales_item_tax_amount'); ?></th>
            <?php
            }
            ?>
        ...
        <?php
        $item_taxes = !empty($item_taxes) ? array_column($item_taxes, null, 'item_id') : [];

        foreach ($cart as $line => $item) {
            if ($item['print_option'] == PRINT_YES) {
                $item_total_price = $item[($this->config->item('receipt_show_total_discount') ? 'total' : 'discounted_total')];
                $item_total_tax = $item_taxes[$item['item_id']]['item_tax_amount'];
        ?>
                <tr style="margin-top:5px;border-top:1px solid #5c5c5c;">
                    ...
                    if ($this->config->item('receipt_show_taxes')) {
                    ?>
                        <td><?php echo to_currency($item_total_price - $item_total_tax); ?></td>
                        <td><?php echo to_currency($item_total_tax); ?></td>
                    <?php
                    }
                    ?>
                    <td class="total-value"><?php echo to_currency($item_total_price); ?></td>
                    <?php
                    if ($this->config->item('receipt_show_tax_ind')) {
                    ?>
                ...
                ?>
                    <tr>
                        ...
                        ?>
                            <td colspan="5" class="discount"><?php echo to_currency($item['discount']) . " " . $this->lang->line("sales_discount") ?></td>
                        <?php
                        } elseif ($item['discount_type'] == PERCENT) {
                        ?>
                            <td colspan="5" class="discount"><?php echo to_decimals($item['discount']) . " " . $this->lang->line("sales_discount_included") ?></td>
                        <?php
                        }
                        ?>
        ...
        ?>
            ...
            ?>
                <tr>
                    <td colspan="5" class="total-value"><?php echo (float)$tax['tax_rate'] . '% ' . $tax['tax_group']; ?>:</td>
        ...
        <tr>
            <td colspan="5" style="text-align:right;<?php echo $border ? 'border-top: 2px solid black;' : ''; ?>"><?php echo $this->lang->line('sales_total'); ?></td>
        ...
        ?>
        <tr>
            <td colspan="5" style="text-align:right;"> <?php echo $this->lang->line($amount_change >= 0 ? ($only_sale_check ? 'sales_check_balance' : 'sales_change_due') : 'sales_amount_due'); ?> </td>
